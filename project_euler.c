...
int problem_003() {
	long big_number_1 = 600851475143;
	// long big_number_2 = 142913828922;
	int smol_number = 13195, highest_prime = 0;
	long current_number = big_number_1;
	int square_root = sqrt(current_number);

	for (int i = 2; i <= square_root; i++){
		if (art_thou_prime(i) == 0 && current_number % i == 0) {
			printf("Current Highest Prime: %d\n", i);
			highest_prime = i;
		}

		if (art_thou_prime(current_number / i) == 0 && current_number % i == 0 && current_number / i > highest_prime) {
				highest_prime = current_number / i;
		}
	}
