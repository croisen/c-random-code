...
int main(int argc, char **argv) {
    ...
    bool   paused           = false;
    double time_played_perc = (double)GetMusicTimePlayed(music) / (double)GetMusicTimeLength(music);

    Color colors[13] = {
        BLUE,    GOLD,   GREEN,  LIGHTGRAY, LIME,
    ...
    while (!WindowShouldClose())
    {
        ...
        if (paused == false)
        {
            for (int i = 0; i < 64; i++)
            {
                ...
                (circles[i].alpha > 1.0F) ? circles[i].speed *= -1: circles[i].alpha;
        ...
        time_played_perc = (double)GetMusicTimePlayed(music) / (double)GetMusicTimeLength(music);
        ...
        BeginDrawing();
            ...
            for (int i = 0; i < 64; i++)
            {
                DrawCircleV(circles[i].position,
                            circles[i].radius,
                            Fade(circles[i].color, circles[i].alpha));
            }
            DrawRectangle(windowX/20,
                          windowY - 2*(windowY/30),
                          windowX - 2*(windowX/20),
                          windowY/30,
                          RED);
            DrawRectangle(windowX/20,
                          windowY  - 2*(windowY/30),
